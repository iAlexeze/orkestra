  ...
  ork status -w --interval 2s`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for {
			...
			if err := printStatus(status, crdFilter); err != nil {
				return err
			}

			// Single-shot mode: exit after one render
			if !watch {
				return nil
			}

			// Watch mode: sleep, then re-fetch and re-render
			time.Sleep(interval)
		}
	},
}
...
func init() {
	...
	statusCmd.Flags().BoolP("watch", "w", false, "Watch status and refresh continuously")
	statusCmd.Flags().Duration("interval", time.Second, "Refresh interval when watching")
}

// ── Rendering helpers ────────────────────────────────────────────────────────

// printStatus renders the full status view (header, table, legend) for one snapshot.
func printStatus(status katalogResponse, crdFilter string) error {
	// Header
	...
	// Optional CRD filter (by name or Kind, case-insensitive)
	crds := status.CRDs
	if crdFilter != "" {
	...
	// Last errors (full messages, not truncated)
	if hasErrors {
		fmt.Println()
