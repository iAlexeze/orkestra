...
The same command runs locally and in CI. The e2e.yaml file is the source of truth.
  ...
  ork e2e -f e2e.yaml --cluster my-existing-context`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		currentCtx, _ := cmd.Flags().GetBool("current-context")
		clusterCtx, _ := cmd.Flags().GetString("cluster")

		runner, err := e2e.New(file, clusterCtx, currentCtx, keepCluster)
		if err != nil {
			return err
		}
...
func init() {
	...
	e2eCmd.Flags().Bool("current-context", false, "Use the current kubectl context, skip cluster creation")
