...
func e2eInitSuite(cmd *cobra.Command, args []string) error {
	...
	cwd, _ := os.Getwd()
	suite := e2e.BuildDiscoveryE2E(paths, wait)
	suite.Metadata.Name = "suite"
	suite.Metadata.Description = fmt.Sprintf("Generated by ork e2e init --suite — %d file(s) discovered", len(paths))

	// Relativize all import paths against CWD so the written file is portable.
	for i, imp := range suite.Imports {
		rel, err := filepath.Rel(cwd, imp.Path)
		if err == nil {
			suite.Imports[i].Path = "./" + rel
		}
	}
