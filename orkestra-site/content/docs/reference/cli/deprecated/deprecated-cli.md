...
```bash
ork validate --file <path> [--file <path>...]
```
...
# Remote
ork validate --file https://raw.github.com/myorg/crds/main/katalog.yaml
...
```bash
ork template --file ./katalog.yaml --yaml
```
...
```bash
ork template --file ./katalog.yaml --verbose
```
...
```bash
ork generate registry --file <path> [flags]
```
...
# Multiple sources
ork generate registry --file ./infra.yaml --file ./apps.yaml
```
...
# Remote
ork run --file https://config.company.com/crds/production-katalog.yaml

# Environment variable
ork run --file $KATALOG_PATH
```
...
ork validate --file examples/website/website-katalog.yaml

# 4. Preview what will run
ork template --file examples/website/website-katalog.yaml --graph
...
# 3. Validate
ork validate --file katalogs/my-katalog.yaml

# 4. Generate runtime registry
ork generate registry --file katalogs/my-katalog.yaml

# 5. Install dependencies
go mod tidy

# 6. Run
go run ./cmd/orkestra/ --file katalogs/my-katalog.yaml
```
