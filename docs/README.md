# 📚 Orkestra Documentation

Welcome to the Orkestra documentation hub — the complete reference for the world’s first **zero‑code Kubernetes operator framework**.

Orkestra turns CRDs into operators.  
This documentation explains how.

---

# 🧭 Start Here

If you're new to Orkestra, begin with:

👉 **[Start Here — The Orkestra Onboarding Guide](./start-here.md)**

This gives you the mental model, the workflow, and the first operator you’ll build.

---

# 🧩 Concepts

Core ideas that define how Orkestra works.

- **[Katalog](./katalog.md)** — Declare operator behavior  
- **[Komposer](./komposer.md)** — Compose multiple Katalogs and sources  
- **[Katalog & Komposer Reference](./katalog-komposer-reference.md)** — Full schema  
- **[Templating](./templating.md)** — Template expressions and resolution  
- **[Your CRD is Enough](../publications/your-crd-is-enough.md)** — The philosophy  

---

# 🛠 Guides

Step‑by‑step instructions for using Orkestra.

- **[CLI Reference](./cli.md)**  
- **[Deployment Guide](./deployment.md)**  
- **[Use Cases](./use-cases.md)**  
- **[Inspect Live CRD](./inspect-live-crd.md)**  

---

# 🧱 OrkestraRegistry

The operator standard library — the future of reusable operator patterns.

- **[OrkestraRegistry Vision](./orkestra-registry/orkestra-registry-vision.md)**  
- **[OrkestraRegistry Technical Documentation](./orkestra-registry/orkestra-registry-technical-documentation.md)**  

---

# 🧬 Internals & Architecture

Deep dives into how Orkestra works under the hood.

- **[Architecture](./architecture.md)**  
- **[Komponents](./komponents.md)**  
- **[Health Subsystem](./health-subsystem.md)**  
- **[Metrics](./metrics.md)**  
- **[Dependency Model](./dependency-model.md)**  
- **[Extending Orkestra](./extending.md)**  

---

# 🗺 Roadmap

- **[Roadmap](../ROADMAP.md)** — What’s coming next

---

# 📰 Publications

High‑level papers and conceptual documents.

- **[Why Orkestra](../publications/why-orkestra.md)**  
- **[Declarative Operators Whitepaper](../publications/declarative-operators-whitepaper.md)**  
- **[Your CRD is Enough](../publications/your-crd-is-enough.md)**  
- **[Metrics Analysis](../publications/metrics-analysis.md)**  

---

Orkestra is evolving rapidly — check back often for updates.
