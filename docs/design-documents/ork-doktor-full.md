# ork doktor — Full Design Document
...
- Part 1: Core `ork doktor` expansions (v1.1)
...
## Part 1: Core ork doktor expansions

### 1.1 Rename: doctor → doktor

All references to `ork doctor` become `ork doktor`. The k-theme is consistent
...
ork doktor init --name myproject --notify-me
```

### 1.2 Git metadata and license extraction

`ork doktor init` reads project metadata before generating the Katalog.
...
`ork doktor` scans `.env` for notification credentials. When found, it
...
`ork doktor` and `ork deploy` check for required tools and install missing
...
found, `ork doktor` exits clearly:
```
  ✗ Docker not found
...
Every deployment generated by `ork doktor init` gets a ServiceAccount, Role,
...
When detected, `ork doktor` surfaces them during examination:
...
Examining docker-compose.yaml...
  ...
  Note: Stateful services require ork doktor v1.3 or later.
...
etc. — that `ork doktor` imports when a matching service is detected.
...
```
cd my-app/
ork doktor init --use-compose docker-compose.yaml --notify-me
...
- `ork doktor` rename with backward-compatible alias
