...
`ork doctor` imports these automatically when it detects a known infrastructure
...
# ork doctor detects postgres:16
...
(via `ork doctor`) never see a StatefulSet, PVC, or headless Service. The
...
```yaml
# app.yaml (generated by ork doctor, filled in by developer)
...
- You are using `ork doctor` and the service was in your docker-compose
...
orkestra-motifs
  ...
  ↓ developers use directly via ork doctor
