...
deployment. Three commands — `ork doktor`, `ork doktor init`, `ork deploy` —
...
**Look** — `ork doktor` examines the project directory and surfaces what it
...
**Prepare** — `ork doktor init` generates three files. The first is
...
The Katalog generated by `ork doktor init` uses the same primitives as any
...
engineers and operators generated by `ork doktor init` for developers are the
...
The developer who uses `ork doktor init` and `ork deploy` does not learn:
...
that goal for platform engineers and DevOps practitioners. `ork doktor` and
