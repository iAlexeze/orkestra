...
| Note | Description |
|------|-------------|
| `isValidLabelValue` | Report whether a string is a syntactically valid Kubernetes label value: max 63 characters, alphanumeric, may contain `-`, `_`, `. |
| `isValidLabelKey` | Report whether a string is a syntactically valid Kubernetes label key: `[prefix/]name` — `name` is alphanumeric (max 63 chars, may contain `-`, `_`, `. |
| `isValidAnnotationKey` | Report whether a string is a syntactically valid Kubernetes annotation key — the same `[prefix/]name` format as a label key. |
| `isDNS1123Subdomain` | Report whether a string is a syntactically valid DNS-1123 subdomain: lowercase alphanumeric segments (may contain `-`) separated by `. |
