...
- Set `group` and `plural` in `katalog.yaml` — `kind`, `version`, `location`, `alias`, `object`, `objectList`, owned `managedResources:`, and `watch:` entries are auto-detected from `SetupWithManager`
