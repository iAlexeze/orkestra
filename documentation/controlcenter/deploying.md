...
## 2. Kubernetes Deployment (Helm — simple)

For a quick cluster deployment, generate the bundle from your Katalog first, then install via Helm:
...
| Helm Chart (simple) | Simple cluster deployment | Generate bundle, then `helm install` |
| Helm Chart (full) | Production | Recommended; full configuration, ingress, TLS, scaling |
