...
Yes. Set `ADMIN_USERNAME`, `ADMIN_PASSWORD`, and `SESSION_SECRET` as environment variables (or via Kubernetes Secret with `envFrom`). When these are set, the Control Center requires login before access. For additional protection (SSO, OAuth2), you can also run it behind an authenticating reverse proxy.
