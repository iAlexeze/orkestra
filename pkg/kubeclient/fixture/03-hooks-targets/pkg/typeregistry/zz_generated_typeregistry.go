...
func RegisterRuntimeObjects() {
	...
	// BlockchainAppWithTargets/v2-enabled — per-target Go hook factory
	// Distinct hook binary for this target; TargetHookFactories carries it into startCRDWorkers.
	{
		gvk := schema.GroupVersionKind{Group: "demo.orkestra.io", Version: "v1alpha1", Kind: "BlockchainAppWithTargets"}
		if orktypes.TargetHookRegistry[gvk] == nil {
			orktypes.TargetHookRegistry[gvk] = map[string]func() domain.AnyReconcileHooks{}
		}
		orktypes.TargetHookRegistry[gvk]["v2-enabled"] = func() domain.AnyReconcileHooks {
			return bchooks.BlockchainAppHooks()
		}
	}

	// BlockchainAppWithTargets/v2-disabled — per-target Go hook factory
	// Distinct hook binary for this target; TargetHookFactories carries it into startCRDWorkers.
	{
		gvk := schema.GroupVersionKind{Group: "demo.orkestra.io", Version: "v1alpha1", Kind: "BlockchainAppWithTargets"}
		if orktypes.TargetHookRegistry[gvk] == nil {
		...
		orktypes.TargetHookRegistry[gvk]["v2-disabled"] = func() domain.AnyReconcileHooks {
			return bchooks.BlockchainAppHooks()
		}
	}

	// BlockchainAppWithTargets/v2-ctor — per-target custom reconciler constructor
	...
	logger.Debug().
		...
		Int("targetHookRegistrySize", len(orktypes.TargetHookRegistry)).
		Int("targetRecRegistrySize", len(orktypes.TargetReconcilerRegistry)).
