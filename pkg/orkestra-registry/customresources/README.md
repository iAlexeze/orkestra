# `pkg/orkestra-registry/customresources` – Dynamic Client Operations for Third-Party CRDs
...
- `pkg/orkestra-registry/template/` — the `Resolver` that evaluates Go templates before `Resolve` is called
- `pkg/orkestra-registry/README.md` — top-level contract shared by all registry packages
