...
// DeleteIfOwned deletes the Job if it exists and is owned by the CR.
func DeleteIfOwned(ctx context.Context, kube *kubeclient.Kubeclient,
	owner domain.Object, name, namespace string) error {

	existing, err := kube.Clientset().BatchV1().Jobs(namespace).
	...
	return kube.Clientset().BatchV1().Jobs(namespace).
