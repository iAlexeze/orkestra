...
`pkg/tools/generate` contains one generator per artifact type. All generators follow the same shape: they accept `[]orktypes.CRDEntry` (or derived data), render output, and either write files or print to stdout when `dryRun` is true.
