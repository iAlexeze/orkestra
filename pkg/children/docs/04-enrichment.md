...
4. Register the enrichment key in [builtins.go](../builtins.go) so that `enrichmentEnabled`, `IsValidEnrichmentTarget`, and `SupportedEnrichmentTargets` all know about it:
   - If the key matches the canonical name, plural, or a `Shorthands` entry of the target resource — it is already covered.
   - If the key is a context-specific name that differs (e.g. `"owner"` on ReplicaSet, `"pvcs"` on StatefulSet, `"replicasets"` on Deployment, `"backingpods"` on Service) — add it to `EnrichKeys` on the relevant `BuiltInKind`. `EnrichKeys` declares which enrichment keys this resource responds to as a data provider.
