...
2. Write an `enrichGroupWith<Name>(ctx, kube, m, crd)` function that iterates over the map and embeds data under a `_<name>` key.
3. Add a call in [children.go](../children.go) `ReadChildren` for the relevant resource type(s).
4. Enable ContextEnrichmentTarget on the related BuiltInKind in `builtInRegistry` → [builtins.go](../builtins.go). If the enrichment name differs from the canonical name, plural, or shorthand, add an alias.
5. Add note functions in the matching `kube_<name>.go` file under [pkg/note](../../note/) that navigate the new key.
6. Add a fixture katalog in [pkg/note/fixture](../../note/fixture/) to exercise the notes end-to-end.
