...
`pkg/ork` wraps the Helm operations Orkestra needs to install and configure itself on a cluster. These are thin wrappers around `helm upgrade --install` — not a general-purpose Helm client.
...
| Caller | What it does |
|--------|-------------|
| `pkg/e2e/runner.go` | Installs Orkestra before running E2E test expectations |
