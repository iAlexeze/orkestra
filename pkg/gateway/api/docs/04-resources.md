...
The Control Center Serve form submit, the CC polls the runtime's `/katalog/{crd}/cr/{namespace}/{name}` for status — it already knows the runtime URL, and the response is richer (children, ready extraction, enriched status from the informer cache).
...
With `serve.config.response.poll.field: status.phase` set, `pollUrl` already carries `?field=status.phase` — the response is the scalar value directly, not the full CR:
