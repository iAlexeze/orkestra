...
// newKat builds a minimal *katalog.Katalog with one IDP-enabled CRD
...
// appCRD is a reusable IDP-enabled CRD entry for tests.
...
// restrictedCRD returns a CRD with allowedTokens configured.
func restrictedCRD() *orktypes.CRDEntry {
	crd := appCRD()
	crd.IDP.AllowedTokens.Tokens = map[string]orktypes.IDPTokenPermissions{
		"ci-pipeline": {
...
func TestCheckIDPPermission(t *testing.T) {
	crd := restrictedCRD()

	check := func(token, op, ns string, class orktypes.IDPEndpointClass) int {
		r := httptest.NewRequest(http.MethodGet, "/", nil)
		r = requestWithToken(r, token)
		rr := httptest.NewRecorder()
		ok := ExportedCheckIDPPermission(rr, r, crd, class, op, ns)
		if ok {
	...
	assert.Equal(t, http.StatusOK, check("ci-pipeline", "get", "default", orktypes.IDPClassResources))
	assert.Equal(t, http.StatusOK, check("ci-pipeline", "list", "default", orktypes.IDPClassResources))
	assert.Equal(t, http.StatusOK, check("ci-pipeline", "get", "default", orktypes.IDPClassSchema))
	assert.Equal(t, http.StatusForbidden, check("ci-pipeline", "delete", "default", orktypes.IDPClassResources))

	// read-only: global=[get list] — applies to all classes
	assert.Equal(t, http.StatusOK, check("read-only", "get", "default", orktypes.IDPClassResources))
	assert.Equal(t, http.StatusOK, check("read-only", "get", "default", orktypes.IDPClassSchema))
	assert.Equal(t, http.StatusForbidden, check("read-only", "delete", "default", orktypes.IDPClassResources))

	// unknown token
	assert.Equal(t, http.StatusForbidden, check("rogue", "get", "default", orktypes.IDPClassResources))

	// no restrictions — nil IDP
	openCRD := appCRD()
	openCRD.IDP.AllowedTokens.Tokens = nil
	rOpen := httptest.NewRequest(http.MethodGet, "/", nil)
	rOpen = requestWithToken(rOpen, "anyone")
	rrOpen := httptest.NewRecorder()
	assert.True(t, ExportedCheckIDPPermission(rrOpen, rOpen, openCRD,
		orktypes.IDPClassResources, "delete", "default"))
}
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// IDPTokenPermissions.TokenAllowed — class-aware permission model
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

func TestTokenAllowed(t *testing.T) {
	cfg := func(tokens map[string]orktypes.IDPTokenPermissions) *orktypes.IDPConfig {
		return &orktypes.IDPConfig{
			Enabled:       true,
			AllowedTokens: orktypes.IDPAllowedTokens{Tokens: tokens},
		}
	}
	...
	cases := []struct {
		...
		wantOK     bool
		wantReason orktypes.IDPDenyReason
	}{
		{
		...
		},
		{
			...
			idp: cfg(map[string]orktypes.IDPTokenPermissions{
				"cc": {Permissions: orktypes.IDPPermissionSet{Global: []string{"*"}}},
			}),
		...
		},
		{
			...
			idp: cfg(map[string]orktypes.IDPTokenPermissions{
				"ci": {Permissions: orktypes.IDPPermissionSet{
					Global:    []string{"get", "list"},
		...
		},
		{
			...
			class:      orktypes.IDPClassResources,
			wantOK:     false,
			wantReason: orktypes.IDPDenyReasonOperation,
		},
		{
			name: "falls back to global when class list empty",
			idp: cfg(map[string]orktypes.IDPTokenPermissions{
				"audit": {Permissions: orktypes.IDPPermissionSet{
					Global: []string{"get", "list"},
				}},
			}),
		...
		},
		{
			...
			idp: cfg(map[string]orktypes.IDPTokenPermissions{
				"ci": {Permissions: orktypes.IDPPermissionSet{Global: []string{"*"}}},
			}),
			token: "rogue", op: "get", ns: "default",
			class:      orktypes.IDPClassResources,
			wantOK:     false,
			wantReason: orktypes.IDPDenyReasonUnknownToken,
		},
		{
			...
			idp: cfg(map[string]orktypes.IDPTokenPermissions{
				"ci": {
					...
					Permissions: orktypes.IDPPermissionSet{Global: []string{"*"}},
				},
			}),
			token: "ci", op: "create", ns: "production",
			class:      orktypes.IDPClassResources,
			wantOK:     false,
			wantReason: orktypes.IDPDenyReasonNamespace,
		},
		{
			name: "empty permissions denies",
			idp: cfg(map[string]orktypes.IDPTokenPermissions{
				"empty": {Permissions: orktypes.IDPPermissionSet{}},
			}),
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			ok, reason := tc.idp.TokenAllowed(tc.token, tc.op, tc.ns, tc.class)
