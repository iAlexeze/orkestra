...
// for correctness
func (k *Katalog) validateAdmissionRules() error {
	if err := k.validateMutationRules(); err != nil {
		return err
	}
	if err := k.validateValidationRules(); err != nil {
		return err
	}
...
func (k *Katalog) validateMutationRules() error {
	for name, crd := range k.enabledCRDs {
		// validate serve configuration
		if crd.ServeEnabled() {
			if err := k.validateServeMutationSynth(name); err != nil {
				return err
			}
		}
...
func (k *Katalog) validateServeMutationSynth(crdName string) error {
	for name, crd := range k.enabledCRDs {
		// serve.fields
...
func (k *Katalog) validateValidationRules() error {
	for _, crd := range k.enabledCRDs {
		if !crd.HasValidationRules() {
			continue
		}
