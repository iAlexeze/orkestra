...
func (k *Katalog) Enabled() []runtime.CRDEntry {
	return k.enabledCRDs
}

// Get tries to get an enabled crd
func (k *Katalog) Get(name string) (*runtime.CRDEntry, error) {
	for _, crd := range k.enabledCRDs {
		if crd.Name == name {
			return &crd, nil
		}
