		...
		for tokenName, perms := range crd.IDP.AllowedTokens {
			...
			if _, ok := knownTokens[tokenName]; !ok {
				return fmt.Errorf(
					...
					crdName, tokenName, tokenName,
				)
			}

			// 2. Every permission string must be a valid operation.
			...
			if crd.HasAllowedNamespaces() || crd.HasRestrictedNamespaces() {
				for _, ns := range perms.Namespaces {
					if !crd.IsNamespaceAuthorized(ns) {
						return fmt.Errorf(
							"namespace %q is not authorized for CRD %q. Check %q.allowedNamespaces and %q.restrictedNamespaces.",
							ns, crd.Name, crd.Name, crd.Name,
						)
					}
				}
			}
			...
			if !crd.IsNamespaced() && len(perms.Namespaces) > 0 {
				crd.Warnings.AddWarning(fmt.Sprintf(
...
// gateway.applyAPI.auth.tokens. Returns nil when the gateway is not configured.
