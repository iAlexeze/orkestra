...
Orkestra is a production implementation of the observer model. Each CRD managed
by Orkestra receives a dedicated operator stack: its own informer watching a
specific GVK, its own workqueue with independent depth and backoff, its own
worker pool whose goroutines cannot be consumed by other CRDs, its own
reconciler interpreting only that CRD's templates, and its own health endpoint
and metrics.
...
— is demonstrated in production with zero conversion code written [5]. Conversion
rules are declared alongside reconcile templates and evaluated by the same
resolver. The mechanism that requires an external webhook process in the
standard operator model requires nothing external in Orkestra.

The Katalog declaration includes a named profile system for resource allocation,
probe timing, security context, HPA scaling behavior, PodDisruptionBudget
configuration, and rolling update strategy. Each profile is a named preset
that expands at load time into fully-formed configuration. The standard
interface that the observer model promises extends not just to health and
metrics, but to the operational decisions that previously required per-operator
bespoke configuration.

The `imports.registry` model distributes operator definitions as OCI artifacts.
Consumers import patterns with a URL reference. Structure is validated at pull
time. The ecosystem is growing: OCI is how the Kubernetes community distributes
everything else, and Orkestra's registry follows the same distribution model.
...
CRDs: Production Results*. Orkestra runtime metrics.
`orkestra_conversion_requests_total{result="success"}` — zero failures across
all tracked production deployments.
